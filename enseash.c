...
int main(int argc, char* argv[]){
	...
	char bgprocessmsg[16];

	while(1){
		if(bgtot>0) { //si on a des process en background on affiche combien y'en a 
			sprintf(bgprocessmsg,"[%d&]",bgtot);
			write(STDOUT_FILENO,bgprocessmsg,strlen(bgprocessmsg));
		}
		write(STDOUT_FILENO,"enseash % ",11);
		if((promptlen = read(STDIN_FILENO,prompt,MAXBUF))==-1) exit(EXIT_FAILURE);
...
void command(char prompt[MAXBUF]){
	...
	if(!strncmp(prompt,"exit",5) || strlen(prompt)==0){ //pour exit
		char* msg = "Ciao\n";
	...
	if(!background){ //si on est pas en background
		...
		char stime[32];
		sprintf(stime,"%ld",total_time);

		char exitmsg[32];

		if (WIFEXITED(stat)){ //mort naturelle
			write(STDOUT_FILENO,"[exit:",7);
			sprintf(exitmsg,"%d",WEXITSTATUS(stat));
		}
		else if (WIFSIGNALED(stat)){ //mort par signal
			write(STDOUT_FILENO,"[sign:",7);
			sprintf(exitmsg,"%d",WTERMSIG(stat));
		}

		write(STDOUT_FILENO,exitmsg,strlen(exitmsg));
		write(STDOUT_FILENO,"|",2);
		write(STDOUT_FILENO,stime,strlen(stime));
		write(STDOUT_FILENO,"ms] ",5);
	}
}
	...
	for(int j=0;j<i;j++){
		if(!strncmp(vector[j],">",2)){// on trouve ">", le string après sera le fichier créé.
			...
			if (pid==0){ //l'enfant execute la 1ère commande
				...
				close(pipefd[1]);
				write(STDOUT_FILENO,wrong_command,strlen(wrong_command));
				exit(EXIT_FAILURE);
		...
		/*else if (!strncmp(vector[j],"&&",3)){
			...
			if (pid==0){
				vector[j]==(char*)NULL;
				execvp(vector[j+last_pipe],vector+last_pipe);
				write(STDOUT_FILENO,wrong_command,strlen(wrong_command));
				exit(EXIT_FAILURE);
			} else {
				waitpid(pid,&stat);
